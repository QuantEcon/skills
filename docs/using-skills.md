...
| `qe` | `/qe:workplan-project`, `/qe:workplan-issue`, `/qe:workplan-update` | The work-plan family: turn an audit/review report into a tracking issue with linked sub-issues; create the single work-plan issue that carries state between agent sessions; maintain it across sessions (resume, update, or close-and-succeed). All GitHub writes are drafted first and gated on your approval | merged as complete procedures, no validated run yet — [skills#3](https://github.com/QuantEcon/skills/issues/3) |
