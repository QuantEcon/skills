...
STEPS = [
    ...
    # see scripts/benchmark/calibration/bellman_bench.py
]
...
PLUGIN = (os.environ.get("CLAUDE_PLUGIN_ROOT")
          or os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(LEC_DIR)))))

# Provenance stamp (shared: scripts/benchmark/scoring/env_stamp.py — the seed of the
...
subprocess.run([PY, os.path.join(PLUGIN, "scripts", "benchmark", "scoring", "env_stamp.py"),
                LEC_DIR] + failed, check=False)

# Scoring is shared across lectures: fill ../evidence.json from the results
# above, then apply the common rubric (scripts/benchmark/scoring/rubric.py) via the engine.
print("\n" + "=" * 70 + "\n== Scorecard (shared rubric)\n" + "=" * 70)
subprocess.run([PY, os.path.join(PLUGIN, "scripts", "benchmark", "scoring", "score.py"),
                LEC_DIR], check=False)
