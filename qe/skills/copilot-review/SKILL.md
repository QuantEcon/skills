...
> **Status: operational.** Promoted from a personal skill that had been in daily use, and validated on 2026-08-03 from an installed plugin: plugin-root path resolution, cross-repo mode, and running from outside a working tree. Shipped in [#26](https://github.com/QuantEcon/skills/pull/26); the `qe` plugin's remaining work — the style surface — is tracked in [#3](https://github.com/QuantEcon/skills/issues/3).
