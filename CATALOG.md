...
1. **Canonical source (upstream proposal):** restart `QuantEcon/style-guide` as the single focused human-and-machine-readable rules DB it was designed to be (schema, fixtures, renderers already exist; 3/55 rules transcribed before it stalled 2026-06-11). The manual's `styleguide/` pages become rendered consumers. Needs a proposal in `project-style-guide` / meta — the skill work supplies the momentum and the transcriptions.
2. **Vendored snapshot (in this plugin):** self-contained `references/rules/` copy structured per the style-guide schema (`qe-*` IDs, `mode: mechanical|hybrid|llm`, `severity`, `build_risk`, `auto_fix`, `exclusions`), seeded from the action's 49 rule prompts reconciled against the July-2026 manual. A `scripts/sync-rules.py` + CI drift check keeps it aligned with upstream once the DB is live — until then, rules written here **seed the DB** (same schema, same files), so skill work and programme restart are the same work, not a fork.
