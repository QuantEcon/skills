...
|---|---|---|---|
| **`qe`** — author-facing skills, from drafting a lecture to merging its PR | `/qe:copilot-review`; `/qe:check-style`, plus six per-category siblings: `check-writing`, `check-math`, `check-code`, `check-figures`, `check-jax`, `check-refs` | Mixed. `/qe:copilot-review` runs: it is a working personal skill promoted here, but it has never run from an installed plugin, so `${CLAUDE_PLUGIN_ROOT}` resolution and the cross-repo mode are unvalidated in that context. The style skills are still scaffolding — the rendered rule snapshot and the deterministic preflight have not landed, and they report that they are not yet operational when run. | [#3](https://github.com/QuantEcon/skills/issues/3) |
